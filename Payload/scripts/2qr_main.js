...
function chunk_base64(base64){
    ...
    document.getElementById('qr_codes_nums').innerHTML = '';
...
function renderQR(chunk, i) {
    setTimeout(function () {
        ...
        var qr_num_id = 'qrnum_' + i;
        var placeholder2 = document.createElement('div');
        placeholder2.id = qr_num_id;
        placeholder2.style.cssText = 'display:none';
        placeholder2.innerHTML = i;
        document.getElementById('qr_codes_nums').appendChild(placeholder2);
        ...
        dats = "d " + i;
        new QRCode(document.getElementById(qr_num_id), {
            width: qr_image_size,
            height: qr_image_size,
            text: dats,
            correctLevel : QRCode.CorrectLevel.H
...
function playback() {
    ...
    setInterval(function () {
        ...
        var all_qrnums = document.querySelectorAll('[id^=qrnum_]');
        if (!cancelPlayback) {
            for (var index = 0; index < all_qrs.length; index++) {
                var qr = all_qrs[index];
                qr.style.cssText = 'display:none';
            }
            for (var index = 0; index < all_qrnums.length; index++) {
                var qrnum = all_qrnums[index];
                qrnum.style.cssText = 'display:none';
            }
            document.getElementById('qrcode_' + current_qr).style.cssText = css_string;
            document.getElementById('qrnum_' + current_qr).style.cssText = css_string;
            ...
            for (var index = 0; index < all_qrs.length; index++) {
                ...
                all_qrnums[index].style.cssText = 'display:none';
            }
            current_qr = 0;
        }
    }, playback_delay);
