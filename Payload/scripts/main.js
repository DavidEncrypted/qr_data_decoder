var qr_string_size = 169;
var qr_image_size = 200;
var playback_delay = 150;
var playback_amount = 10;
...
function playback() {
    ...
    setInterval(function () {
        ...
        if (!cancelPlayback) {
            ...
            for (var showqr = 0; showqr < playback_amount; showqr++){
                var tempqr = current_qr + showqr;
                document.getElementById('qrcode_' + tempqr).style.cssText = css_string;
            }
            //document.getElementById('qrcode_' + current_qr).style.cssText = css_string;
            current_qr += playback_amount;
            if (current_qr > all_qrs.length - playback_amount && current_qr < all_qrs.length) {
                current_qr = all_qrs.length - playback_amount;
            }
            //console.log(current_qr);
            if ((current_qr) >= all_qrs.length - 1) {
                cancelPlayback = true;
            }
        } else {
            for (var index = 0; index < all_qrs.length; index++) {
